...
-- Batch 5: Justification requests table
IF OBJECT_ID(N'dbo.JUSTIFICACIONESTAJAMAR', N'U') IS NOT NULL DROP TABLE dbo.JUSTIFICACIONESTAJAMAR;
GO

CREATE TABLE [dbo].[JUSTIFICACIONESTAJAMAR](
    ...
    CONSTRAINT [UQ_JUSTIFICACION_ASISTENCIA] UNIQUE ([IDASISTENCIA])
) ON [PRIMARY];

ALTER TABLE [dbo].[JUSTIFICACIONESTAJAMAR] WITH CHECK ADD CONSTRAINT [FK_JUSTIFICACION_ASISTENCIA]
    FOREIGN KEY([IDASISTENCIA]) REFERENCES [dbo].[ASISTENCIATAJAMAR] ([IDASISTENCIA]);

ALTER TABLE [dbo].[JUSTIFICACIONESTAJAMAR] WITH CHECK ADD CONSTRAINT [FK_JUSTIFICACION_PROFESOR]
    FOREIGN KEY([IDPROFESOR]) REFERENCES [dbo].[USUARIOSTAJAMAR] ([IDUSUARIO]);

CREATE NONCLUSTERED INDEX [IX_JUSTIFICACION_ESTADO] ON [dbo].[JUSTIFICACIONESTAJAMAR] ([ESTADO] ASC)
    INCLUDE ([IDASISTENCIA]);
GO

PRINT 'Migration completed: ROLESCHARLASTAJAMAR, CURSOSTAJAMAR, USUARIOSTAJAMAR, CURSOSUSUARIOSTAJAMAR, ASISTENCIATAJAMAR, JUSTIFICACIONESTAJAMAR.';
